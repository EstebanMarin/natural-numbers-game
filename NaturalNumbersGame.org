...
To set up and build this project, run the following commands:

#+begin_src bash
# Download mathlib cache (this will take a few minutes the first time)
lake exe cache get
...
1. *Option 1 - Use the Org file directly:* Open this file in Emacs and use ~C-c C-v t~ to tangle it, which will generate ~NaturalNumbers/Game.lean~ with all the code blocks.

2. *Option 2 - Use the separate Lean files:* The project includes pre-made files:
   - ~NaturalNumbers/Tutorial.lean~
   - ~NaturalNumbers/Addition.lean~
   - ~NaturalNumbers/Multiplication.lean~

   Open any of these in VS Code with the Lean 4 extension.

3. *Solving exercises:* Replace ~sorry~ with your proofs. Lean will automatically check them and show green checkmarks for valid proofs or red squiggles for errors.
...
Unverified
9:02 AM
when to use simp? exact? omega? or should we just play som type tetris here?
